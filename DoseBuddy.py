...
st.set_page_config(page_title="💊🩹DoseBuddy AI Assistant🤖", layout="centered")
...
with st.sidebar.expander(" 🛠️ Settings ", expanded=False):
    ...
    evaluate_flag = st.selectbox(
        label="Require Ragas Evaluation",
        options=[
            "Yes",
            "No"
        ],
...
if user_query is not None and user_query != "":
    ...
    with st.chat_message("AI", avatar="🤖"):
        answer = st.write_stream(get_answer(user_query, st.session_state.chat_history[-2*K:]))

        print(f'tool is {tool}')
        if evaluate_flag == 'Yes':
            if tool == 'RAG':
                with st.spinner("Evaluating..."):
                    print(formulated_question)
                    ground_truth = get_gt(user_query, context, st.session_state.chat_history[-2*K:])
                    answer_relevancy,answer_correctness = ragas_evaluate(formulated_question,context,answer,ground_truth)
                    # answer_relevancy,answer_correctness = 0.66, 0.777
                    st.markdown(f'<i> <div style="text-align: right;color: orange; font-size:13px;font-weight:bold; font-family:Arial, Helvetica, sans-serif;"> relevancy {answer_relevancy:.3f}  correctness {answer_correctness:.3f}</div> </i>', unsafe_allow_html=True)
